...
fn pure_prismatic_chain_jacobian_linear_only() {
    ...
    let joint_types = vec![JointType::Prismatic(Axis::X), JointType::Prismatic(Axis::Y), JointType::Prismatic(Axis::Z)];
    ...
    let mb: MultiBody<3,3> = MultiBody::new(offsets, Some(mass_mats), None, None, joint_types, parent, na::Vector3::zeros(), None, None, None, None, None).unwrap();

    let base = na::Isometry3::identity();
    let joint_vars = na::SVector::<f64,3>::from_vec(vec![0.0,0.0,0.0]);
    let conf = mb.minimal_to_homogeneous_configuration(&base,&joint_vars);
    ...
    for (i,axis) in [na::Vector3::x(), na::Vector3::y(), na::Vector3::z()].iter().enumerate() {
