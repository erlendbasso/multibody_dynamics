...
fn zero_mass_body_contributes_nothing() {
    ...
    let mass1 = 2.0; let mass2 = 0.0; // zero mass edge case
    let r1 = na::Vector3::new(0.0,0.0,0.0); let r2 = na::Vector3::zeros();
    ...
    let r_com = vec![r1,r2];
    let mb: MultiBody<2,2> = MultiBody::new(offsets,None,None,Some(inertia),joint_types,parent,na::Vector3::zeros(),Some(r_com),None,Some(masses),None,None).unwrap();

    let base = na::Isometry3::identity();
    let q = na::SVector::<f64,2>::zeros();
    let conf = mb.minimal_to_homogeneous_configuration(&base,&q);
