...
fn single_sixdof_body_mass_matrix_matches_input() {
    ...
    let joint_vars = na::SVector::<f64,6>::zeros();
    ...
    expected.fixed_view_mut::<3,3>(0,0).copy_from(&(mass * na::Matrix3::identity()));
    expected.fixed_view_mut::<3,3>(0,3).copy_from(&(-mass * skew(&r_cg)));
    expected.fixed_view_mut::<3,3>(3,0).copy_from(&(mass * skew(&r_cg)));
    expected.fixed_view_mut::<3,3>(3,3).copy_from(&inertia_com);
